...
namespace Bode.DataAccess.Sql
{
    public class SqlDataQueryBuilder : IDataQueryBuilder
    {
        ...
        {
            return $"INSERT INTO [{table}] ({string.Join(",", insertFields.Select(f => $"[{f.ColumnName}]"))}) VALUES ({string.Join(",", insertFields.Select(f => $"{ToParameterName(f.PropertyName)}"))}) SELECT SCOPE_IDENTITY()";
        }
