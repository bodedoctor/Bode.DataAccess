...
{
    public class MySqlDataQueryBuilder : IDataQueryBuilder
    {
        ...
        public string Insert(string table, IEnumerable<Binding> insertFields)
        {
            return $"INSERT INTO `{table}` ({string.Join(",", insertFields.Select(f => $"`{f.ColumnName}`"))}) VALUES ({string.Join(",", insertFields.Select(f => $"{ToParameterName(f.PropertyName)}"))}); SELECT LAST_INSERT_ID();";
        }
